...
class Camera:
    ...
    def _raw_render(self, sprites: List[Sprite]) -> np.ndarray:
        ...
        for sprite in sorted_sprites:
            ...
            sprite_region = sprite_pixels[sprite_y_start:sprite_y_end, 
    ...
    def render(self, sprites: List[Sprite]) -> np.ndarray:
        ...
        output = np.full((self.MAX_DIMENSION, self.MAX_DIMENSION), 
                        self._letter_box, dtype=np.int8)
        ...
        output[y_offset:y_offset + view.shape[0], 
               x_offset:x_offset + view.shape[1]] = view
